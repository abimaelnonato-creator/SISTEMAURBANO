...
const GEMINI_API_KEY = 'AIzaSyAtVxAsEcbS3qmKG_Zh6t1CkW7oH_Aopxo';
...
export async function sendChatMessage(
  ...
  const systemContext = getSystemContext(systemData);
  ...
  // Adicionar instrução do sistema no primeiro conteúdo
  if (geminiContents.length > 0 && geminiContents[0].role === 'user') {
    geminiContents[0].parts[0].text = `${systemContext}\n\n---\n\nPergunta do usuário: ${geminiContents[0].parts[0].text}`;
  }
